...
import { basename, relative, resolve, sep, dirname, join, isAbsolute } from "node:path";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Process a local video or URL into a timestamped FlameFrame evidence pack.",
		promptSnippet: "Process a video URL or local file into a timestamped evidence pack.",
		promptGuidelines: [
			"Use flameframe_process when the user asks to inspect, summarize, search, or answer questions about a video.",
		],
		parameters: Type.Object({
			input: Type.String({ description: "Local video path or HTTP(S) URL." }),
			workDir: Type.String({ description: "Deterministic output directory for the evidence pack." }),
			budget: Type.Optional(Type.Number({ description: "Maximum selected frames to write." })),
			fps: Type.Optional(Type.Number({ description: "Low-resolution analysis FPS." })),
			segmentSeconds: Type.Optional(Type.Number({ description: "Segment length in seconds." })),
			windowSeconds: Type.Optional(Type.Number({ description: "Transcript markdown window size in seconds." })),
			timeoutSeconds: Type.Optional(Type.Number({ description: "URL download timeout in seconds." })),
			maxHeight: Type.Optional(Type.Number({ description: "Maximum downloaded video height." })),
			subLangs: Type.Optional(Type.String({ description: "Caption languages, such as en,en-orig." })),
			noCaptions: Type.Optional(Type.Boolean({ description: "Do not download captions for URL input." })),
		}),
		async execute(_toolCallId, args, signal) {
			await runFlameFrameProcess(args, signal);
			return {
				content: [{ type: "text", text: `Processed FlameFrame evidence pack: ${args.workDir}` }],
				details: {},
			};
		},
	});
	...
	pi.registerCommand("flameframe-browser", {
		...
		handler: async (args, ctx) => {
			...
			await browseSessionPacks(sessionPacks, ctx, pi);
		},
	});
}

async function browseSessionPacks(packs: SessionPack[], ctx: ExtensionContext, pi: ExtensionAPI): Promise<void> {
	if (packs.length === 0) {
		ctx.ui.notify("No FlameFrame packs are registered in this session. Run flameframe_process or /flameframe-browser <pack-or-workdir>.", "info");
		return;
	}
	if (ctx.mode !== "tui") {
...
class FrameBrowser implements Component {
	...
	private previewLines(width: number): string[] {
		...
		const status = loading && this.previewFrameId !== selectedFrame?.frameId
		...
		const padding = Array.from(
			{ length: Math.max(0, BROWSER_PREVIEW_RESERVED_ROWS - imageLines.length) },
			() => "",
		);
...
function renderPreviewPng(path: string): Promise<Buffer> {
	return new Promise((resolvePreview, rejectPreview) => {
		const child = spawn(
			"ffmpeg",
			["-v", "error", "-i", path, "-frames:v", "1", "-f", "image2pipe", "-vcodec", "png", "pipe:1"],
...
class FrameImageOverlay implements Component {
	...
	render(width: number): string[] {
		...
		const visibleTranscript = transcript.slice(
			...
			this.transcriptOffset + DETAIL_TRANSCRIPT_VISIBLE_LINES,
		);
		return roundedPanel(
			[
				...
				"",
				this.theme.fg("muted", ` Transcript ${this.transcriptOffset + 1}-${this.transcriptOffset + visibleTranscript.length}/${transcript.length} `),
				...visibleTranscript,
				"",
...
function packsFromSession(ctx: ExtensionContext): SessionPack[] {
	...
	for (const entry of ctx.sessionManager.getBranch()) {
		const details = entry.type === "message" && entry.message.role === "toolResult" && entry.message.toolName === PROCESS_TOOL_NAME
...
async function runFlameFrameProcess(input: ProcessToolInput, signal?: AbortSignal): Promise<void> {
	const args = ["process", input.input, "--work-dir", input.workDir];
	appendProcessOption(args, "--budget", input.budget);
	appendProcessOption(args, "--fps", input.fps);
	appendProcessOption(args, "--segment-seconds", input.segmentSeconds);
	appendProcessOption(args, "--window-seconds", input.windowSeconds);
	appendProcessOption(args, "--timeout-seconds", input.timeoutSeconds);
	appendProcessOption(args, "--max-height", input.maxHeight);
	appendProcessOption(args, "--sub-langs", input.subLangs);
	if (input.noCaptions) args.push("--no-captions");

	await new Promise<void>((resolveProcess, rejectProcess) => {
		const child = spawn("flameframe", args, { stdio: ["ignore", "ignore", "pipe"] });
		const errors: Buffer[] = [];
		let errorBytes = 0;
		let settled = false;
		...
		const abort = () => {
			child.kill();
			finish(new Error("FlameFrame process cancelled"));
		};
		child.stderr?.on("data", (chunk: Buffer) => {
			if (errorBytes >= MAX_CONTEXT_BYTES) return;
			errors.push(chunk.subarray(0, MAX_CONTEXT_BYTES - errorBytes));
			errorBytes += chunk.length;
		});
		child.once("error", (error) => finish(new Error(`could not start flameframe: ${error.message}`)));
		child.once("close", (code) => {
			if (code === 0) finish();
			else finish(new Error(Buffer.concat(errors).toString("utf8").trim() || `flameframe process exited with status ${code ?? "unknown"}`));
		});
...
function appendProcessOption(args: string[], name: string, value: number | string | undefined): void {
